...
- (void)requestAPIDataSpport:(int)supportType  success:(void (^)(PAIPAModel *apiModel))success failure:(void (^)(NSError *error))failure{
    ...
    NSString *dataPath = [[NSBundle mainBundle] pathForResource:@"params" ofType:@"json"];
    if (supportType == 2) {
        dataPath = [[NSBundle mainBundle] pathForResource:@"params2" ofType:@"json"];
    }
...
//    }];
    ...
    NSString *httpUrl = @"http://pa-engine.zplayads.com/v1/api/ads";
      ...
      completionHandler:^(NSURLResponse *_Nonnull response, id _Nullable responseObject, NSError *_Nullable error) {
          ...
          NSDictionary *dictFromData = [NSJSONSerialization JSONObjectWithData:responseObject
                                                                       options:NSJSONReadingAllowFragments
                                                                         error:&error];
          PAIPAModel *model = [PAIPAModel yy_modelWithJSON:dictFromData];
          success(model);
      }] resume];
}
